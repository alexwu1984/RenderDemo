...
void FCommandContext::CopyBuffer(FD3D12Resource& Dest, FD3D12Resource& Src)
{
...
void FCommandContext::CopyBufferRegion(FD3D12Resource& Dest, size_t DestOffset, FD3D12Resource& Src, size_t SrcOffset, size_t NumBytes)
{
	TransitionResource(Dest, D3D12_RESOURCE_STATE_COPY_DEST);
	//TransitionResource(Src, D3D12_RESOURCE_STATE_COPY_SOURCE);
	FlushResourceBarriers();
...
void FCommandContext::CopySubresource(FD3D12Resource& Dest, UINT DestSubIndex, FD3D12Resource& Src, UINT SrcSubIndex)
{
	FlushResourceBarriers();
