...
std::shared_ptr<MeshData> FObjLoader::LoadObj(const std::wstring& FilePath, bool FlipV)
{
	double StartTime = FTimer::GetSeconds();
	std::wcout << std::endl << L"loading: " << FilePath << std::endl;
	...
	std::shared_ptr<MeshData> meshdata(new MeshData(FilePath));
	...
	while (getline(Buffer, Line))
	{
		...
		else if (*line_str == 'v')
		{
			if (isspace(*(line_str + 1)))
			{
				...
				positions.push_back(z);
				meshdata->UpdateBoundingBox(Vector3f(x, y, -z), vMinf3, vMaxf3);
			}
		...
		else if (strncmp(line_str, "mtllib", 6) == 0)
		{
			...
			BasePath += ToWideString(line_str);
...
bool FObjLoader::LoadMaterialLib(MaterialLibType& MtlLib, const std::wstring& MtlFilePath)
{
	MtlLib.clear();
	...
	while (getline(Buffer, Line))
	{
		...
		{
			std::wstring BasePath = GetBasePath(MtlFilePath);
		...
		else if (strncmp(line_str, "Ka", 2) == 0)
		{
			line_str += 2;
			ParseFloat3(line_str, color_r, color_g, color_b);
			CurrentMaterial.Ka = Vector3f(color_r, color_g, color_b);
		}
		else if (strncmp(line_str, "Kd", 2) == 0)
		{
			line_str += 2;
			ParseFloat3(line_str, color_r, color_g, color_b);
			CurrentMaterial.Kd = Vector3f(color_r, color_g, color_b);
		}
		else if (strncmp(line_str, "Ks", 2) == 0)
		{
			line_str += 2;
			ParseFloat3(line_str, color_r, color_g, color_b);
			CurrentMaterial.Ks = Vector3f(color_r, color_g, color_b);
		}
...
bool FObjLoader::ParseMap(const char* &str, const std::wstring& base_path, const std::string& map_key, std::wstring& OutPath)
{
	if (strncmp(str, map_key.c_str(), map_key.length()) == 0)
	{
		str += map_key.length();
		SkipToNoneSpace(str);
		OutPath = base_path + ToWideString(str);
		return true;
	}
	return false;
}
...
std::wstring FObjLoader::GetBasePath(const std::wstring& FilePath)
{
	std::wstring::size_type Pos = FilePath.rfind('/');
...
MaterialData FObjLoader::CreateMaterialFromName(MaterialLibType& MtlLib, const std::string& MaterialName)
{
	...
	if (MtlLib.find(MaterialName) != MtlLib.end())
	{
		...
		Material.Ka = objMat.Ka;
		Material.Kd = objMat.Kd;
		Material.Ks = objMat.Ks;
	...
	else
	{
		...
		Material.BaseColorPath = L"../Resources/Textures/white.png";
	}
	return Material;
}
