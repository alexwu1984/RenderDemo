...
TextureCube IrradianceCubeMap : register(t5);
TextureCube PrefilteredCubeMap: register(t6);
Texture2D PreintegratedGF 	: register(t7);
Texture2D OpacityMap: register(t8);
...
void PS_PBR_GBuffer(PixelInput In, out PixelOutput Out)
{
    ...
    float Metallic = Roughness_MetallicMap.Sample(LinearSampler, In.Tex).b;
    float Roughness = Roughness_MetallicMap.Sample(LinearSampler, In.Tex).g;
