...
Texture2D SSDOTexture : register(t0);
...
{
    float2 SSDOTexSize;
    SSDOTexture.GetDimensions(SSDOTexSize.x, SSDOTexSize.y);
    float2 TexelSize = 1.0f / SSDOTexSize;
    float3 Result = 0;
    for (int x = -2; x < 2; ++x)
    {
        for (int y = -2; y < 2; ++y)
        {
            ...
            Result += SSDOTexture.Sample(LinearSampler, PIN.tex + Offset).rgb;
        }
    }
