...
void BaseRenderPass::Init(const std::vector < std::shared_ptr<FRenderItem>>& ItemList, int Width, int Height)
{
	m_ItemList = ItemList;
...
void BaseRenderPass::Render(FCommandContext& CommandContext)
{
...
void BaseRenderPass::Update(const Vector3f& LightDir, const FMatrix& View, const FMatrix& Proj, FCamera& MainCamera)
{
	for (auto Item : m_ItemList)
	{
		auto Model = Item->Model;
...
void BaseRenderPass::SetupRootSignature()
{
...
void BaseRenderPass::SetupPipelineState()
{
