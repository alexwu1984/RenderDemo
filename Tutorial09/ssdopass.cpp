...
void SSDOPass::Init(int Width, int Height)
{
...
void SSDOPass::Render(FCommandContext& CommandContext, FColorBuffer& PositionBuffer, FColorBuffer& NormalBuffer, FColorBuffer& AlbedoBuffer)
{
	// Set necessary state.
	...
	CommandContext.TransitionResource(AlbedoBuffer, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
	...
	CommandContext.SetDynamicDescriptor(4, 0, AlbedoBuffer.GetSRV());

	//m_RenderItem->Geo->Draw(CommandContext);
	CommandContext.Draw(3);
}

FColorBuffer& SSDOPass::GetAOBuffer()
{
	return m_AOBuffer;
}

void SSDOPass::SetupRootSignature()
{
	...
	m_RootSignature[4].InitAsDescriptorRange(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 3, 1, D3D12_SHADER_VISIBILITY_PIXEL);
	m_RootSignature.InitStaticSampler(0, DefaultSamplerDesc, D3D12_SHADER_VISIBILITY_PIXEL);
	m_RootSignature.Finalize(L"SSDOPassSignature", D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT);
}
...
void SSDOPass::SetupPipelineState()
{
	...
	m_AOBuffer.Create(L"SSDO Buffer", m_GameWndSize.x, m_GameWndSize.y, 1, DXGI_FORMAT_R32G32B32A32_FLOAT);

	m_AOPassConstBuf.CreateUpload(L"SSDOPassInfo", sizeof(SSDOPassInfo));
	m_AOPassCpuHandle = m_AOPassConstBuf.CreateConstantBufferView(0, sizeof(SSDOPassInfo));

	m_PassInfo.KernelSize = 64;
	m_PassInfo.Radius = 1.0;
	m_PassInfo.IndirectLightScale = 5.0;
...
std::vector<Vector4f> SSDOPass::GenerateNoise()
{
	std::uniform_real_distribution<float> RandomFloats(0.0, 1.0); // 随机浮点数，范围0.0 - 1.0
	std::default_random_engine generator;
